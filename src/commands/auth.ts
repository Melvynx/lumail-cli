...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  lumail-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  lumail-cli auth show\n  lumail-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: lumail-cli auth set <token>");
      return;
    }
